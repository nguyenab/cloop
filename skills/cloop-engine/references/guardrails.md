...
- `last_progress_iteration` — integer. The last iteration that flipped a criteria box, or — when no
  ledger exists — landed a non-trivial commit. Set it whenever real progress lands.
...
When `iteration % 5 == 0`, the Planner re-reads the plan's Goal and the criteria ledger verbatim
before scoping, and records one line in the ADR Context: whether the recent trajectory still serves
the goal, and anything that drifted. A deterministic counter, not a similarity score.
